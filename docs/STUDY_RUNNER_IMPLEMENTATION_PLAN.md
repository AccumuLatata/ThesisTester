...
**Study Admit Follow-up (separate SAF series):** drafting a linked child StudySpec with Admit locked to a post-hoc ToD bucket is specified in `docs/STUDY_ADMIT_FOLLOWUP_IMPLEMENTATION_PLAN.md` (SAF1 CLI `--admit-tod auto` + SAF2 Inspect draft shipped; SAF3 not started). That series does **not** reopen RS5 “promote never executes” or this series’ execute loop. Default `study promote` (flags omitted) stays identical. Do not implement SAF inside an RS PR.
