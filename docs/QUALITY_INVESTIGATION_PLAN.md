...
- No code, test, fixture, config, dependency, or workflow edits. The **only** file exceptions to research-only are (1) QI-0's additive harness (`scripts/quality_metrics.py`, `docs/quality/README.md`, `docs/quality/findings.csv` header) and (2) QI-15's draft `docs/QUALITY_REMEDIATION_PLAN.md`. QI-1…QI-14 have none. The §4.6 CI hotfix is **not** a QI slice and is not authorized by this document.
...
1. **Research only.** Slices QI-1…QI-14 create files under `docs/quality/` only. QI-0 may add the harness files in §1.2; QI-15 may add `docs/QUALITY_REMEDIATION_PLAN.md`. Those two exceptions are the only tracked-file exceptions. Running code, tests, probes, and throwaway scripts under `/tmp` is allowed and expected; committing them is not.
2. **Regression-safe.** `pytest -q` must be identical before and after a slice (no tracked file changed, except the §1.2 exceptions). Probe tests live in `/tmp` and are pasted into the report as evidence.
3. **Drift-safe.** Stay inside the slice's `Scope` file list. **Exclusive file ownership:** every path is owned by exactly one slice (QI-0's map is the SoT). Adjacent observations go into the report's `Handoffs` section with the target slice ID — never into another slice's scope.
...
| Maintainability Index per module | `radon mi -s` | **MI < 20** → module is a refactor candidate; **MI = 0.00** → structural finding | Eight pages plus `analytics/confluence_attribution.py` score 0.00 today (§4) |
...
| `st.session_state` fan-in/fan-out | `rg -c 'st\.session_state' pages` per page, plus key-level producer/consumer graph | keys not in the `ARCHITECTURE.md` contract table | 1,176 matching lines; the contract table is the ground truth |
| Test → module traceability | coverage per module (`--cov-report=term-missing`), plus manual mapping of the 196 test `.py` files to the 16 slices | any library module < 70% or any public function with 0 tests | Suite-wide coverage is high but unevenly distributed |
| Test wall-time | `pytest --durations=25` | any single test > 5 s; any file > 60 s | Feedback-loop cost is a maintainability metric |
| Determinism | run the slice's suite twice with `-p no:cacheprovider` and once with `PYTHONHASHSEED` varied; diff outputs | any difference | Repo policy: no dict-order or wall-clock dependence. **Do not** pass `-p no:randomly` — `pytest-randomly` is not a project dependency |
...
| Dimension | Value |
|---|---|
| Library LOC (`thesistester/`) | 81,829 in **153** modules |
| Streamlit pages LOC (`pages/` only) | **18,172** in 15 pages. `app.py` is 51 lines and is owned by QI-10, not this count |
| Test LOC / files / functions | 92,644 / **196** `.py` files / **3,715** `def test_` (**3,966** collected) |
...
| Maintainability Index = **0.00** | **Eight pages** (`pages/1_Data.py`, `3_Setup_Builder.py`, `6_Signals.py`, `7_Backtest.py`, `10_Validation.py`, `14_Research_Assistant.py`, `15_Studies.py`, `16_Study_Observatory.py`) **plus** `analytics/confluence_attribution.py`; `pages/2_Levels.py` 7.24; `analytics/overfitting.py` 16.09 |
...
| `st.session_state` matching lines in pages | 1,176 (Studies 235 · Data 170 · Validation 140 · Research Assistant 138) |
| Broad `except Exception` / bare `except` | 83 |
| `TODO/FIXME/XXX/HACK` | **0 real.** Case-insensitive `rg` hits 8, all `todo` local variables in `study/execute.py` (cell work-list), not markers |
| `# noqa` / `# type: ignore` | 36 |
...
- **H-0b (verified). The coverage gate is silently below its own floor.** 82% measured vs 85% floor vs 88% R9 baseline (§A.4). The `::warning` annotation in CI is the only signal and nobody acts on it; the debt is in the assistant/voice, classic bridge, CLI, and journal layers, not the engine. → QI-11 (where the missing assertions matter), QI-12 (floor policy), QR-B (blocking floor at the measured level, then ratchet).
...
- **H-B. Pages are the under-tested half.** Eight pages have MI 0.00 (plus `analytics/confluence_attribution.py`). Streamlit `AppTest` is used for exactly two pages (`tests/test_assistant_page_render.py` → page 14; `tests/study/test_study_observatory.py` → page 16); the thirteen classic/Studies/Journal pages are covered only via `*_page_helpers` unit tests of extracted helpers. The one `AppTest` suite is also the one that broke on the Streamlit minor bump (§4.3). → QI-10, QI-11.
...
- **H-E. Assistant code volume exceeds engine code volume.** The highest CC in the repo is `reporting.build_markdown_report` **152** (QI-6); the assistant's densest function is `_format_scalar_for_claim` 120. Whether assistant volume is proportionate is a QR decision; QI-9 measures it. → QI-9.
...
This plan PR **does not** implement the hotfix. QI's guardrail 2 ("`pytest -q` identical before and after") is meaningless while `main` is red, and every slice would re-report the same two failures. Therefore, **before any slice starts**, one narrow hotfix PR (not a QI slice, not QR, not this document's PR; it follows `ENGINEERING_PROPOSAL` §4.2 like any PR) should:
...
Sixteen slices (**QI-0…QI-15**). **QI-0 first; QI-1…QI-14 are mutually independent and may run in parallel; QI-15 last.** Each investigation slice is one agent-run and one PR. QI-1…QI-14 add only their report(s) to `docs/quality/` and rows to `findings.csv`. QI-0 may add the §1.2 harness files; QI-15 may add `docs/QUALITY_REMEDIATION_PLAN.md`.
...
**Probes:** `streamlit run app.py --server.headless true` boots and serves `/_stcore/health`; `python -m thesistester --help`, `python -m thesistester study --help`, and `python -m thesistester journal --help` succeed. There is no standalone `study` / `journal` console script.
...
**Scope:** `thesistester/data/` (`loader.py` incl. R17 vendor profiles, `derive.py`, `resample.py`, `rolls.py`, `sessions.py`, `quantower_ticks.py`), `thesistester/persistence/local_store.py` (exclusive owner of the file; dataset *and* levels namespaces), `pages/1_Data.py` (46 defs, 170 session refs), `config.py` instrument presets, `sample_data/`.
...
**Scope:** `thesistester/levels/` (`all.py` orchestrator, `common.py`, `session_date.py`, `sessions.py`, `indicators.py`, `pivots.py`, `session_vwap.py` incl. w/mVWAP, `tpo.py` single prints, `profile.py`, `apoc.py` + `apoc_candidates.py` + `apoc_tick.py`, `rolling_poc_candidates.py` + `rolling_poc_tick.py`, `tick_vap.py` + `tick_requirements.py`, `prev30m_vwap.py`, `defaults.py`, `catalog.py`), `visualization/levels_chart.py`, `pages/2_Levels.py`. `local_store.py` is QI-1-owned (levels-namespace observations are a QI-1→QI-2 handoff). `docs/POINT_IN_TIME_GUARANTEES.md` is QI-13-owned, read as spec.
...
**Scope:** `thesistester/api.py` (3,204 lines), `cli.py`, `__main__.py`, `research_identity.py`, `research_bundle.py` (`load_research_bundle` CC 75, `build_research_bundle` CC 63), `reporting.py` (`build_markdown_report` CC 152 — highest CC in the repo), `classic_*.py` except `classic_nav.py` (QI-10), `persistence/execution_artifacts.py` (1,519 lines), `app_state.py`, pages `11_Report_Export.py`, `12_Research_Bundles.py`.
...
**Scope:** `thesistester/study/` (`schema.py` 973 — `_validate_factors` CC 46; `expand.py`; `execute.py` 1,353 — `run_study` CC 60; `report.py`; `promote.py` — `_compose_promoted_draft` CC 37; `rollup.py`; `viewer.py` 1,174; `observatory.py` 2,069; `builder.py` 1,386 — `hydrate_study_draft` CC 62; `launch.py` 691; `briefing.py`; `admit_followup.py`; `cli_study.py`; `tools.py`), `pages/15_Studies.py` (2,261 lines, 42 defs, 235 session refs), `pages/16_Study_Observatory.py` (966), `examples/studies/**` incl. Program B generator/validator scripts, `docs/STUDY_RUNNER.md` as spec (QI-13 owns the file). `tests/study/` is QI-11-owned.
...
**Scope:** `thesistester/journal/` (`schema.py`, `tradesviz`/AMP loaders, `pair.py`, `reconcile.py`, `join.py` — `_join_trade` CC 33, `levels.py`, `zones.py`, `match.py` — `load_named_cell` CC 35, `counterfactual.py`, `rules.py`, `report.py` — `_q4_q6` CC 36, `cli.py`, `tag_map.yaml`), `pages/17_Journal.py`, `examples/journal/`. `tests/fixtures/journal` is QI-11-owned.
...
**Scope:** `app.py`, `classic_nav.py`, `timezone_display.py`, `.streamlit/config.toml`. Individual `pages/*.py` internals are **exclusively** owned by QI-1…QI-9; `app_state.py` is QI-6-owned (Streamlit-in-library handoff). This slice owns cross-page behavior only and does not own page files. Specs (QI-13-owned): `docs/ARCHITECTURE.md` §"`st.session_state` contract", `docs/USER_GUIDE.md`.
**Why:** 1,176 `st.session_state` matching lines, one prose contract table, `AppTest` coverage for only two of fifteen pages (14 and 16); the audit's restore-leftover class (H1) is a UI-state-lifecycle problem.
...
**Scope:** `pyproject.toml`, `requirements.txt`, `.github/workflows/ci.yml`, `.devcontainer/`, `.env.example`, `scripts/**` (except the QI-0 harness exception), `LICENSE`, `config/assistant.toml`, gitignore of `config/assistant.voice.override.toml`, secret paths in `assistant/llm.py`. `.streamlit/` is QI-10-owned; root `README.md` is QI-13-owned (read install/run sections as spec).
...
**Scope:** `docs/**` (66 files at `e82c2a9`; exclusive owner of every markdown file), root `README.md`, `.cursor/rules/thesistester.mdc`. Public-API docstrings (`api.py` facade, CLI help) are read as spec; those files are QI-6-owned.
...
QI-15 is signed when: (a) all fourteen investigation-slice reports (QI-1…QI-14) exist and each has a `Positive verification` section; (b) `findings.csv` parses and every row has `disposition ∈ {open, closed-verified, locked, wont-fix, duplicate}`; (c) the CTO review (regression and architecture drift first, per repo rules) has classified every `High`+ finding as accepted/deferred/rejected with a one-line reason; (d) the draft QR plan cites finding IDs for every workstream.
...
- PR body must include: commit audited, commands run, finding count by severity, `Positive verification` summary, and the research-only sentence. For QI-1…QI-14: "No tracked file outside `docs/quality/` changed; `pytest -q` unchanged." QI-0 additionally names the §1.2 harness files. QI-15 additionally names `docs/QUALITY_REMEDIATION_PLAN.md`. Those two are the only file exceptions.
...
1. Research only. For QI-1…QI-14 you may create exactly: docs/quality/QI-<nn>_<slug>.md and append rows to docs/quality/findings.csv. Do not edit, create, delete, or reformat any other tracked file. (QI-0's harness files and QI-15's docs/QUALITY_REMEDIATION_PLAN.md are the only plan-wide exceptions; do not use them on this slice.) Probe scripts live under /tmp and are pasted into the report.
...
3. Stay inside the Scope file list for QI-<nn>. Exclusive file ownership: do not edit or claim files owned by another slice. Adjacent observations go to the report's "Handoffs" section with the target slice ID.
...
| **QR-G Dependency and supply-chain hygiene** | QI-12 | **Required status checks on `main`** (`ruff`, every `pytest` cell, editable install, golden guard) so §4.3 cannot recur; lockfile/constraints, dependabot or renovate with full-suite gating, `pip-audit`/`bandit` CI jobs (warn-first), devcontainer alignment, explicit pandas-major and Streamlit-minor matrix/caps decision | Tooling-only; version bumps as separate test-gated PRs (proposal §7) |
...
|---|---|---|
| Merges to `main` with a red required check | 37 in the week to 2026-09-08 | 0 (enforced by required status checks) |
| Dependency-induced test failures detected before merge | 0 of 2 | 100% (renovate/dependabot PR runs the suite before the bump reaches `main`) |
| F-grade functions in `engine/`, `analytics/`, `api.py` | 11 | 0 (E allowed only with a documented reason) |
| Pages with MI 0.00 | 8 (plus `analytics/confluence_attribution.py`) | 0 |
...
| Coverage gate | informational floor 85%; **measured 82%** (R9: 88%) | blocking at the measured level, ratcheted upward per release; no module in `engine/`, `analytics/`, `data/`, `levels/` below 85% |
...
# size (expected on e82c2a9: 153 modules / 81,829 LOC; 18,172 pages/ LOC; 196 test .py / 3,715 def test_)
...
rg -n '\bTODO\b|\bFIXME\b|\bXXX\b|\bHACK\b' thesistester pages   # 0 real markers; do not use -i (matches `todo` locals)
rg -c 'type: ignore|noqa' thesistester pages | awk -F: '{s+=$2} END {print s}'
rg -c 'st\.session_state' pages | awk -F: '{s+=$2} END {print s}'   # matching lines, not unique keys
...
**Exclusive file ownership.** Every path appears in exactly one row. Spec docs listed in a slice's Scope are *read as spec* and are owned by QI-13. Test/fixture files are QI-11 even when they exercise another slice's code. Namespace notes are investigation focus, not dual ownership.

| Path | Slice |
|---|---|
| `thesistester/data/**`, `pages/1_Data.py`, `config.py`, `persistence/local_store.py` | QI-1 |
| `thesistester/levels/**`, `visualization/levels_chart.py`, `pages/2_Levels.py` | QI-2 |
...
| `api.py`, `cli.py`, `__main__.py`, `research_identity.py`, `research_bundle.py`, `reporting.py`, `classic_*.py` except `classic_nav.py`, `app_state.py`, `persistence/execution_artifacts.py`, `pages/11_*.py`, `12_*.py` | QI-6 |
| `thesistester/study/**`, `pages/15_*.py`, `16_*.py`, `examples/studies/**` | QI-7 |
...
| `app.py`, cross-page behavior (not the page files), `classic_nav.py`, `timezone_display.py`, `.streamlit/` | QI-10 |
| `tests/**` (exclusive), `pyproject [tool.pytest|coverage]` | QI-11 |
| `pyproject.toml`, `requirements.txt`, `.github/**`, `.devcontainer/**`, `LICENSE`, `.env.example`, `scripts/**` | QI-12 |
| `docs/**`, `README.md`, `.cursor/rules/**` | QI-13 |
| cross-cutting measurement (no exclusive source files) | QI-14 |
...
| With branch coverage (`--cov=thesistester`, `branch = true`) | 5:16 wall time. **TOTAL 82%** (35,592 statements / 5,133 missed; 14,990 branches / 3,118 partial) across 153 measured modules — **below the CI "informational" floor of 85% and six points below the R9 baseline of 88%**. Because the floor only warns, this drift has never blocked or been surfaced |
| Modules < 70% coverage | `__main__.py` 0 · `assistant/handlers.py` 46 · `assistant/voice/sidecar.py` 48 · `classic_nav.py` 59 · `cli.py` 59 · `levels/common.py` 59 · `classic_record.py` 61 · `assistant/voice/xai_realtime.py` 63 · `assistant/voice/grounding.py` 66 · `classic_ledger.py` 66 · `classic_context.py` 67 · `classic_proposal.py` 67 · `journal/rules.py` 68 · `journal/ledger.py` 69 |
| Most missed statements (absolute) | `assistant/voice/sidecar.py` 262 · `assistant/results_overview.py` 226 · `api.py` 144 · `study/observatory.py` 142 · `assistant/orchestrator.py` 141 · `study/execute.py` 140 · `persistence/execution_artifacts.py` 128 · `journal/report.py` 114 · `assistant/handlers.py` 112 · `assistant/voice/session.py` 111 |
| Engine/analytics coverage | all `engine/*` and `analytics/*` modules ≥ 77% (`engine/signals.py` 84%, `analytics/noise.py` 77%, `analytics/confluence_attribution.py` 81%); the coverage debt sits in the assistant/voice, classic bridge, CLI entry points, and journal — not the simulation core |
