...
- **DA3 (landed):** `simulate_trades(..., same_bar_opposite_direction=...)` is
  ...
  unchanged.
- **DA4 (landed):** new trigger tokens `fade` and `continuation`. Direction is
  implied by the previous **trigger-timeframe** close vs the zone
  (`approach_side`: `above` / `below` / none). A signal emits only on
  touch + a defined approach; exactly one direction per zone per bar.
  `direction` filters that implied side. Optional
  `trigger_params.require_close_confirmation` (default false) adds reject-like
  close confirmation for fade and far-edge close-through for continuation.
  `approach_side` is attached after `_make_signal` on fade/continuation rows
...
- Backtest entry for simple triggers (`touch`, `reject`, `break`, `reclaim`, `fade`, `continuation`) remains `bar_index + 1` on the canonical/base dataframe (first base bar after trigger-candle completion).
...
- **H14:** Non-base `3c` remaps the zone `bar_index` / `timestamp` onto the completed HTF bar and keeps developing-partner prices (dVWAP / SMA / rolling VWAP) at the **early-window** value tested against that HTF OHLC. Decision T is HTF close (not a future-bar leak). Simple HTF keeps `base_end` zones only. Snap-to-`base_end` is a parked CTO decision (QR plan §7.2), not current behavior.
...
- `_normalize_levels_settings` and `_sync_levels_widget_state` fill missing keys from `DEFAULT_LEVELS_SETTINGS` and sort the same list fields via `thesistester.levels.defaults.canonicalize_levels_list_fields` / `LEVELS_SORT_KEYS` (QI-02-04 / D-6). Extra snapshot keys are kept; `instrument` is not injected. The three planes stay distinct (`compute_all_levels` kwargs remain additive/off).
- `pivot_timeframes` is sorted deterministically in normalization (same treatment as `sma_lengths`, `ema_lengths`, `sma_timeframes`, `ema_timeframes`, `vwap_windows`, `poc_windows`).
- `_sync_levels_widget_state` restores opt-in controls (including `prev30mVWAP`) when a saved snapshot is loaded. Old snapshots missing newer keys fill those controls from the product table and are treated as settings-stale until recalculate (stored frame was not computed under omit-means-on).
- Snapshot labels show stored agg ticks or `—` when omitted (no third-table `1`). Opt-in suffix lists only families explicitly enabled in the stored dict.
...
- **M10.** Grid ranking is an in-sample sort, not a contest. The WFA RdYlGn
  heatmap is a robustness surface of median OOS expectancy — do not treat the
  top ranked cell or the greenest matrix cell as a production selection. OTF
  matrix train-selected chrome is diagnostic (no trophy). Ranking math is
  unchanged.
