...
- `_normalize_levels_settings` and `_sync_levels_widget_state` fill missing keys from `DEFAULT_LEVELS_SETTINGS` and sort the same list fields via `canonicalize_levels_list_fields` / `LEVELS_SORT_KEYS` (QI-02-04 / D-6). Extra snapshot keys are kept; `instrument` is not injected. The three planes stay distinct (`compute_all_levels` kwargs remain additive/off).
