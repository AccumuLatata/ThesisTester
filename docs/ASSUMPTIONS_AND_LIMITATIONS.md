...
- R17 vendor parsing is selected explicitly; ThesisTester does not infer a
  ...
  allow-list (blank / unknown → `canonical`). Omitting the key is no longer a
  builder path; CLI YAML that still omits it keeps the runner default
  `canonical` until re-emitted.
