...
- `_normalize_levels_settings` and `_sync_levels_widget_state` fill missing keys from `DEFAULT_LEVELS_SETTINGS` and sort the same list fields as `normalize_levels_config` (QI-02-04 / D-6). Extra snapshot keys are kept; `instrument` is not injected. The three planes stay distinct (`compute_all_levels` kwargs remain additive/off).
...
- `_sync_levels_widget_state` restores opt-in controls (including `prev30mVWAP`) when a saved snapshot is loaded. Old snapshots missing newer keys load safely and fill those controls from the product table.
