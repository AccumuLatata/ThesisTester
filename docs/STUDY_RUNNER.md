...
**SV** (Study Viewer) SV1–SV3 ✅
`docs/STUDY_VIEWER_IMPLEMENTATION_PLAN.md`. SV0–**SV3** shipped (catalog +
`study list` + click-to-load + quality panes + overview charts). SV4 not
started. Does not change preview / CLI-spawn / execute. Inspect remains
artifacts-only (`report_study(..., write_artifacts=False)`; no `rollup_study()`).
...
**Status:** SV0–**SV3** shipped. SV4 not started. Plan:
...
**Until SV4 ships:** cell peek stays ranked tables / CLI files. Paste
`output_dir` remains valid.
