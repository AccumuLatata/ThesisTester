...
`primary_metric` is in-sample only: `expectancy_r` / `total_r` /
`max_drawdown_r` / `trade_count` / `profit_factor`. Index / rollup may store
`wfa_median_test_expectancy_r` when WFA ran; that column is **not rankable**
...
- **H16:** `wfa_median_test_expectancy_r` is stored on `results_index.csv` / rollup when present; it is **not rankable** as `primary_metric` and does not rank or promote. Allowlist stays in-sample (QI-05-06).
