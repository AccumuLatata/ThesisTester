...
**SAF** (Study Admit Follow-up) — SAF1–SAF3 shipped
`docs/STUDY_ADMIT_FOLLOWUP_IMPLEMENTATION_PLAN.md`.
`study promote --admit-tod auto` drafts a linked child StudySpec with Admit
locked to the briefing NY `entry_rth_segment`. `--tod-group` hour/30min and
`--allow-thin` are CLI-only. Inspect **Draft Admit follow-up** stays RTH +
thin-refuse and writes Preview YAML only. Catalog / `study list` show a
...
**Level-as-anchor combination protocol (Program A — desk funnel):**
...
OTF is the Validation matrix, not `factors.otf`. Holistic level-class ×
confluence-class concept (Program B, not this funnel):
[`docs/LEVEL_COMBINATION_RESEARCH_CONCEPT.md`](LEVEL_COMBINATION_RESEARCH_CONCEPT.md).
Do not treat the teaching
...
import `SETUP_CONFIG_RULES` / `RUN_SPEC_RULES`). Supported / required factor
axes and expand required-cell re-checks (`STUDY_EXPAND_REQUIRED_AXES`) are
derived from that factor table. Omitted `ingestion_mode` still means the
ingest-row `omit_means` (`primary`, AH §2 item 9); Quantower + omit/`omit_means`
...
| `partner_levels` | non-empty list of partner-sets (lists). A set may be `[]` only when `confluence_mode` is exclusively `anchor_rules` **and** `constants.min_valid_confluences` is an explicit `0`. Stage filter/explicit_cells may name that empty set when it is in the factor domain |
...
**not** `study run`. `study expand` prints
`Replay: python -m thesistester run …/experiment.yaml  # <that clause>`
(bash-safe: the command before `  #` is copy-pasteable) and prefixes
`experiment.yaml` with `# Replay: <same clause>`; overview MD repeats the
clause under Honesty (QI-07-06). Study runs do **not** call `run_batch`; they loop
...
| `study.overview.md` | Ranked / low-N / Failed / group summaries / OTF Δ + honesty block |
...
| Program B lens (SO3) ✅ | `desk_class`, ΔE vs Wave 0 (`w0_solo` / `w0_va` / `w0_apoc`), thinning, heatmap when `progB_*` present | Manifest-as-inventory (both `manifest.yaml` and `manifest_tick.yaml` are chrome only); Admit auto-promote; writing onto the Program A desk map |
...
**Status:** SAF1–SAF3 shipped. SAF4 parked.  
...
the briefing NY `entry_rth_segment`. `--tod-group` / `--allow-thin` stay
CLI-only. Inspect **Draft Admit follow-up** writes RTH YAML onto Preview
(does **not** `study run`). Catalog / `study list` show `parent` from lineage.
