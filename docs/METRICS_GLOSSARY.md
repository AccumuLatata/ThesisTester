...
| `exit_rule_delta` | Per declared bracket: `Σ cf_net_ticks − Σ net_ticks` on the same entries. Positive = the mechanical rule beats realized exits (exit leak); negative = the desk's exits add value. Qty-scaled dollar-ticks; AMP fees applied. Caption must say three brackets were looked at. Never mix 15s and tick rows. |
| `entry_edge_flag` | `True` when the best-bracket mean `cf_net_ticks` is `> 0` with n ≥ 30 **per resolution**. Not a single pre-registered test. |
