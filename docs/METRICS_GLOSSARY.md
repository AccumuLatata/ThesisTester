...
| `tag_alignment` | TJ6 verification of **level-class** tags only: `all_aligned` / `partial` / `none_aligned` / `unverifiable`. A tag is aligned when \|`tag_distance_ticks`\| ≤ `tag_tolerance_ticks` (default **10**). Missing token → `tag_level_missing`. Context (ITR/CTR family, `touch`, `3c`) / confirm / unmapped tags never drive this field. Distance check, not a trigger. |
