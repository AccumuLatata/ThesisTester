...
✅ **Implemented.** See `docs/ENGINEERING_ROADMAP.md`. R17 adds explicit
vendor profiles, canonical one-minute trade-capture aggregation, raw capture
sidecars, and micro futures presets without changing the default CSV contract.
...
| pandas/numpy major-version drift | R9 | Named CI pandas-major axis (`pytest (py3.10)` = pandas 2; py3.11/3.12 = pandas 3) + `constraints.txt` + caps in `pyproject.toml`; Dependabot PRs gated by the G-1 required checks (QI-12-02 / QR G-2) |
| Streamlit minor-version drift (AppTest / widget proto) | R9 | `streamlit>=1.56,<1.64` in `pyproject.toml` (pinned 1.63.x in `constraints.txt`); next minor is a Dependabot PR on the full matrix (#478 / QI-12-02) |
