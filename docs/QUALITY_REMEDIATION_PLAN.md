...
**Date:** 2026-09-12. Rev 1 = QI-15 draft ([#494](https://github.com/AccumuLatata/ThesisTester/pull/494), `main @ 29167c2`). **Rev 2 (this document) = fix-vs-close re-baseline:** every finding is either scoped into a concrete PR, kept `locked` (disclosure/test residual only), or closed (`wont-fix` / `duplicate`) with a one-line rationale mirrored in `docs/quality/findings.csv`. **Review-pass (same day):** restored QR guardrails (§1.4); corrected same-file sequencing, A-4/B-19 gate inversion, QR-A golden-hash owners, High-row exit wording, Low kept-open list, and §7.1/§8 census arithmetic. `findings.csv` dispositions unchanged.
...
**Inputs (SoT order):** `docs/quality/findings.csv` (134 rows; `disposition` / `qr_workstream` / `merge_group` / `score` columns) · `docs/quality/QI-15_SYNTHESIS.md` (views, scores, CTO proposals — historical; §2–§4 census and §9 wave list there are the QI-15 snapshot) · `docs/quality/QI-01…QI-14` (probe recipes cited per PR) · `docs/QUALITY_INVESTIGATION_PLAN.md` §3, §7.5, §9 · `docs/ENGINEERING_PROPOSAL.md` §4.
...
- Tooling / CI gates introduced warn-first (§4), except G-1 which restores the existing required-check gate immediately.
...
### 1.4 Guardrails (all QR PRs)

Rev 1 numbered these as §2. They remain binding on every QR PR; census moved to §2 so the CSV is adjacent to the PR tables.
...
2. **Sequencing rules of `QUALITY_INVESTIGATION_PLAN.md` §9.2 are non-negotiable**, applied as the waves and hard rules in §4: QR-A before anything touching the same file; QR-B before QR-C engine/WFA extracts; warn-first gates; one hot spot per PR with CC/MI before/after; docs in the same PR; no feature scope creep from QR-C/D. **Allowed exception:** C-1 (declarative-validator pilot, not an engine file) may start in wave 2 after B-16; it does not waive B-3+B-4 for `signals.py` / `walk_forward.py` / `backtest.py` / `sim_core.py`.
3. **Locked rows are premises.** A QR PR that changes the behavior of a `disposition=locked` row is rejected at review. Disclosure copy and lock-the-fork tests for those rows are allowed and listed in §3.
...
Kept-open Low rows (17; not closed because a MUST-FIX trigger applies): QI-04-10 (financial misread: `pnl_points` vs net R) · QI-09-06 (Security risk) · QI-07-09 (Security risk, rides G-4) · QI-05-13 (refactor-gate: `validation_summary()` freeze, B-19) · QI-06-12 / QI-11-05 / QI-11-06 / QI-12-09 / QI-10-08 (test/tooling gates for later refactors) · QI-01-06 / QI-04-09 / QI-05-14 / QI-06-11 / QI-14-09 (ride inside named hot-spot PRs) · QI-03-09 / QI-03-11 / QI-09-10 (cheap riders in wave 5 PRs that touch the same widgets).
...
**Regression-safety shape:** one PR per finding (or per `MG`); probe becomes a committed test; §4.2 checklist; **no golden regeneration** — A-7 (additive managed keys) and A-17 (error propagation) must leave `trades` and bundle hashes byte-identical on goldens. A-8 is dataset-switch / `display_timezone` invalidation, not a hashed bundle member.
...
| A-2 | QI-04-06 (M) — MG-02 | `pages/7_Backtest.py` (run persist), `docs/ARCHITECTURE.md` | Persist `direction_collision_diagnostic` (additive, unhashed session key) and caption it next to the skip table. Default is persist+caption; "classic UI does not show DA1" is not this PR (§7.2). | `tests/test_backtest_direction_collision.py` extension: page helper stores `candidate_pairs` after a `return_result` run | `ARCHITECTURE.md` session-key row; `METRICS_GLOSSARY.md` DA1; `USER_GUIDE.md` Exposure | key additive; AH4 managed set decision recorded (clear-only) |
...
| A-4 | QI-05-05 (H) — MG-06; delivers QR-F QI-13-03 | `pages/10_Validation.py` (permutation `st.success`, `P(mean R > 0)` metric), `thesistester/reporting.py` (`## Validation Diagnostics` banner string), `docs/METRICS_GLOSSARY.md` | `st.success` → `st.info`/caption; rename label; add diagnostic banner line in export. `validation_summary()` shape untouched (B-19 later freezes the four-key set; A-4 must not wait on B-19). | Copy guard on page 10 + `tests/test_phase9_reporting.py` banner assert | `METRICS_GLOSSARY.md` rows: probability_positive, Phase 8 permutation p-value, grid-overfit Best/Median/delta; `USER_GUIDE.md` Validation; `ASSUMPTIONS` H13 | `validation_summary()` keys unchanged vs HEAD; report fixtures updated for banner only |
...
**Exit:** the four High `app` rows (QI-04-02, QI-05-04, QI-05-05, QI-05-09) and the three Medium `Verified defect` rows (QI-06-03, QI-10-01, QI-08-03) carry `fixed_in`; composer-parity ledger (`QI-15_SYNTHESIS.md` §5.3) has 0 open entries whose fix is disclosure; every locked fork has a disclosure on both composers. (High *docs* QI-13-01 is F-1 / wave 0; High *code* rows are QR-C; High *Verified defect* QI-12-01/QI-12-02 are G-1/G-2.)
...
| B-19 | QI-05-13 (L) + contract C12 | `tests/test_validation.py` (key-set assert) | Mechanize the `validation_summary()` four-key freeze (`bootstrap` / `permutation` / `trade_count` / `grid_overfit`). Contract C9 (`sim_core` ↛ admission/analytics) lands with B-10, not this PR. | QI-5 §6.2 | `AGENT_GUIDE.md` battery `schema_version` table | asserts green; A-4 already required the shape to be unchanged |
...
**Regression-safety shape:** golden-gated **byte-identical** outputs (legacy + B-3 families); one hot spot per PR; CC/MI before → after in the PR body; no public signature change; §4.1 applies; **no `GOLDEN_REGEN` label may appear on a QR-C PR**; C-14…C-20 (every touch of `signals.py` / `walk_forward.py` / `backtest.py` / `sim_core.py`) blocked until B-3 + B-4 exit criteria hold.
...
| C-3 | QI-07-03 (M) — MG-17 | `thesistester/study/schema.py` (`validate_study_spec`, `_validate_factors`, `_validate_constants`) | Declarative factor/report/ingest tables; expand re-checks reuse them. Omitted-key defaults unchanged (AH §2 item 9). **After A-14** (same `schema.py`). | `tests/study/test_study_schema.py` (+ one row per supported axis) | `STUDY_RUNNER.md` §RS1; `AGENT_GUIDE.md` §Study | CC 46 → ≤ 20; Program B manifests validate clean |
...
| C-6 | QI-05-14 (L), QI-06-11 (L) — MG-29 | `thesistester/analytics/grid.py`, `thesistester/analytics/overfitting.py`, `thesistester/analytics/sensitivity.py`, `thesistester/analytics/otf_validation.py`, `thesistester/research_bundle.py` (`_hash_dataframe`), `pages/11_Report_Export.py` | Promote `_directional_grid_metrics`, `_SIMULATION_KWARGS`, `_default_otf_filter_config`, `_hash_dataframe` to public helpers (thin wrappers); inline/export `_dash_if_none`. Prerequisite for C-13/C-17 extracts. After A-17 (same `otf_validation.py`) and A-7/A-18 (same `research_bundle.py`). | existing directional/OTF/sensitivity asserts; `tests/test_golden_master.py` hash | `AGENT_GUIDE.md` analytics public surface; `ARCHITECTURE.md` hash projection | private cross-module imports 10 → ≤ 5 (`rg` count) |
| C-7 | QI-06-04 (M) — MG-01 | `thesistester/research_bundle.py` (18 key tables → one registry) | One registry: section → files → session keys → managed? → hashed?; generate `_MANAGED_RESEARCH_KEYS`, `_*_META_KEYS`, `_KNOWN_FILES`, `_SECTION_REQUIRED_FILES`, hash exclusions. **After A-7 / A-8 / A-18** (same file). | Registry completeness test; `tests/test_research_bundle.py`; golden bundle hash | `ARCHITECTURE.md` bundle members | canonical bundle hash unchanged; CC 75/63 → ≤ 25 |
...
| C-18 | QI-04-09 (L) — MG-18 | `thesistester/engine/backtest.py` (token constants next to `_SKIPPED_SIGNAL_COLUMNS`), `thesistester/analytics/entry_window.py` | Centralize skip/exit tokens as constants; string values unchanged. Prepares C-19. **After B-3 + B-4** (touches `backtest.py`). | Invariant test: every emit site ∈ frozen set; `partition_skip_counts` covers it | `USER_GUIDE.md` skip list only if values change (they must not) | goldens untouched |
...
| C-22 | QI-09-02 (M) | `thesistester/assistant/explainer.py`, `thesistester/assistant/help_corpus.py`, `thesistester/assistant/orchestrator.py` (`handle_results_turn`) | Table-drive `_derive_caveats` and `score_corpus_chunk`; split `handle_results_turn` phases; channel contracts (RQ/HC/DI) unchanged. After C-21 and A-19 (same `orchestrator.py`). | explainer / help_corpus / help_coverage / eval suites | `AGENT_GUIDE.md` assistant surface | CC 52/51/40 → ≤ 25 |
...
| C-24 | QI-07-07 (M) | `thesistester/study/observatory.py`, `thesistester/study/viewer.py`, `thesistester/study/builder.py` | Observatory join vs desk vs lens split; builder hydrate/emit helpers; Streamlit/Plotly stay on pages; read-only guarantees kept (SO). After A-14 (same `builder.py`). | `tests/study/test_study_observatory.py` (post B-12), `test_study_viewer.py`, `test_study_builder.py` | `ARCHITECTURE.md` SO/SV module map | MI 0.00 → > 0 on the three modules; zero writes into study dirs |
...
| E-3 | QI-06-07 (M) — MG-24 | `thesistester/cli.py` (`main`), `thesistester/__main__.py` | Catch `ValueError`/`OSError` for the `run` verb; print `str(exc)`; return `EX_DATAERR` / `EX_NOINPUT`. **After A-9** (same `cli.py` help text). | `tests/test_cli.py`: missing file / empty runs → rc ≠ 0, no `Traceback` in stderr | `AGENT_GUIDE.md` CLI errors | `study` / `journal` verbs unchanged |
...
| E-6 | QI-09-10 (L) — MG-12; residual of locked QI-06-06 | `pages/14_Research_Assistant.py` (Open exact caption), `pages/12_Research_Bundles.py` | Three-bar vocabulary (page-12 schema-only · assistant hash-fail-closed · open-exact) on both pages. No hash gate on page 12. **After A-21** (same Bundles page). | Copy assert on both pages; `test_ah4_p5_page_12_stays_schema_only` unchanged | `USER_GUIDE.md` Research Bundles + Open exact; `ARCHITECTURE.md` persistence bars | bars remain mechanically distinct |
| E-7 | QI-03-09 (L) | `pages/3_Setup_Builder.py` (naked controls) | Reuse the Signals any/all zone-level help on Setup Builder naked controls. **After A-12** (same page); rides with A-12 if timing allows. | Copy guard | `USER_GUIDE.md` Setup Builder naked filters | admission unchanged (M5) |
...
| PR | Findings | Files (expected) | Narrow change | Probe → regression test | Docs in same PR | Exit / merge gate |
|---|---|---|---|---|---|---|
| G-1 | QI-12-01 (H, Verified defect) — MG-13 | GitHub branch protection (settings), `docs/AGENT_GUIDE.md` | **Required status checks on `main`** for the six existing job names (`ruff (lint + format)`, `pytest (py3.10)`, `pytest (py3.11)`, `pytest (py3.12)`, `editable install (no dev extras)`, `golden-master regeneration guard`); job names frozen; no rename in the same change. | `gh api repos/…/branches/main/protection` shows the six checks | `AGENT_GUIDE.md` §Regression-safety gates in CI | blocking immediately (restores §4 rule 9) |
...
| G-3 | QI-12-03 (M) — MG-14 | `requirements.txt`, `README.md`, `.devcontainer/devcontainer.json` (install line) | `requirements.txt` = compiled export of the `pyproject` caps (or deleted; README → `pip install -e .`); `pytest` in the dev extra only. | Optional CI diff of `pip-compile(requirements)` vs `pip-compile(pyproject)` | `README.md` install; `AGENT_GUIDE.md` Fast start | one install SoT |
...
| **1 — honesty first** (copy/disclosure/fail-closed; one PR per finding unless a real `MG-*` shares one change) | A-1 … A-22 (+ F-4/F-7/F-8 delivered inside A-4/A-12/A-9; F-6 with A-22) · B-1 · B-2 | Every High `app` row; H1 residual leftovers; fail-closed gaps; disclosure on both composers for locked forks with their lock-the-fork tests | four High `app` rows `fixed_in`; B-1/B-2 green |
| **2 — safety net** (tests/tooling) | B-3 … B-19 · G-4 · G-5 · **C-1 pilot** (after B-16; validator only — not an engine extract) | Golden families, mutation baseline, PIT generated FS, coverage floor, import-linter/mypy warn-first, AppTest rules, markers, ruff B, CAI harness, scanners | **B-3 + B-4 exit criteria**: 8/8 golden paths; `backtest.py` and `walk_forward.py` own-file mutation ≥ 70 % |
...
**Hard rules:** QR-A before any C/D/E PR touching the same file (A-1 → A-2 → A-20 → A-21 → A-22 → D-3 on `pages/7_Backtest.py`; A-3…A-6 → D-4; A-8/A-22 → D-1 and D-5; A-12 → D-7; A-12/A-13/C-4 → D-8; A-7/A-8/A-18 → C-6/C-7; A-4 → C-5; A-10 → A-15 on `study/report.py`; A-14 → C-3 and C-24; A-17 → C-6; A-19 → C-22; A-9 → E-3; A-12 → E-7; A-21 → E-6) · C-7 → D-1 (bundle registry before session-key registry consumes `_MANAGED_RESEARCH_KEYS`) · C-2 → D-5 (both touch `pages/1_Data.py`) · B-16 before C-1/C-3 · B-3 + B-4 before C-14…C-20 · C-1 before C-2/C-3/D-7 · C-6 before C-13/C-17 · C-14 before C-15/C-16 · C-18 before C-19 · C-19 before C-20/E-10 · B-10 before F-9 · B-18 before F-10 · G-1 before the docs say "blocking" again · every new gate warn-first for one release, blocking in its own PR (G-1 excepted: restores the existing required-check gate) · one hot spot per PR with CC/MI before → after · docs in the same PR · no feature scope creep from C/D PRs.
...
- [ ] QR-C: CC and MI before → after in the PR body; no public signature change; byte-identical outputs on golden + feature-path fixtures; B-3/B-4 gates met for any touch of `signals.py` / `walk_forward.py` / `backtest.py` / `sim_core.py`.
...
| Mutation score (four sampled files) | 100 / 83.3 / 66.7–72.7 / 50–60 | ≥ 80 % each; ≥ 70 % before any QR-C touch of `backtest.py` / `walk_forward.py` / `signals.py` / `sim_core.py` (C-1 validator pilot is exempt) | B-4 |
...
Medium `Verified defect` rows proposed accepted in the same signature: QI-06-03 (A-7), QI-10-01 (A-8), QI-08-03 (A-16). **Rejected: none.** Closures in §2.3 are **12 new `wont-fix` + 1 new `duplicate`** (13 rows). Census totals remain 13 `wont-fix` (includes pre-existing QI-10-06) and 3 `duplicate` (includes pre-existing QI-05-11, QI-10-02).
...
| DA1 persist vs "classic UI does not show" | QI-04-06 | A-2 **persist+caption** (additive unhashed key); hiding DA1 is not this PR |
...
| QR-G Dependency / supply chain | Not started | 5 | 6 / 0 / 0 |
| Closed (no workstream) | — | — | 13 wont-fix · 1 locked without residual (QI-07-10) |
| **Total** | | **102 PR rows, 99 distinct** | **106 open / 12 locked (11 residual + QI-07-10) / 3 duplicate / 13 wont-fix = 134** |
