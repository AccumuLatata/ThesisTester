...
- [ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md](ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md) (RP — Quantower parity for `POC_rolling_30min`; **RP0 locked (rev 2), RP1–RP2 scoped**; next RP1 comparator; print window `[now-W+1min, now+1min)`; RP2 merge-gated on written sliding-window oracle; default stays typical)
