...
- [ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md](ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md) (RP — Quantower parity for `POC_rolling_30min`; **RP1 harness landed**; next RP1g scorecard; print window `[now-W+1min, now+1min)`; RP2 merge-gated on written sliding-window oracle; default stays typical)
