...
| [DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md](DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md) | DA — `touch`+`both`+`single_position` is long-only; **DA0 locked**; DI is Discuss Intelligence |
...
- [DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md](DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md) (DA — Directional Attribution; **not** Discuss Intelligence DI. `touch` + `both` + `single_position` is a long-only sample; **DA0 locked**; DA1–DA6 not started; no golden regen)
