...
- [DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md](DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md) (DI — `touch` + `both` + `single_position` is a long-only sample; collision diagnostic, direction split, opt-in same-bar policy, `fade`/`continuation` triggers, drift null, Program B Run 2; **DI0 proposed**; no golden regen)
