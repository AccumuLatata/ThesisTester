...
Wall times are **informational**. They are not a correctness claim about P&L, metrics, or Study ranking (`AUDIT_FINAL` §5.1 item 10). R22 remains: any later acceleration must equal serial goldens.
...
`.copy()` / `iterrows` AST counts (hot-path modules only): **68** `.copy()` call sites, **9** `iterrows` (`signals.py` 6, `backtest.py` 1, `intrabar.py` 1, `confluence_attribution.py` 2). `walk_forward.py` 14 copies; `confluence_attribution.py` 20. `sim_core.py` has zero copies and zero `iterrows`.
...
| `levels` | 780 × 50 | 0.315 MB | 0 (47 float64 + tz datetime + 1 str) |
...
| Study `--workers` 1/2/4 | Four cells. `bundle_hash` **byte-identical** across 1/2/4. Wall 2.087 / 1.353 / 0.966 s (shared store → later runs are `levels_hit`). |
| Observatory vs corpus | Linear ~3.2 ms/cell cold (17 ms / 4 cells → 819 ms / 256 cells). Warm-with-`prior` ~0.5×. |
| Page rerun (AppTest, 780-bar session) | First render 0.11–0.32 s; rerun 0.004–0.222 s; 0 exceptions. Pickle of `{data,levels,signals,trades,setup}` = **0.818 MB** vs `.streamlit` `maxMessageSize=400`. |
...
H4 (two levels-settings planes) is QI-2-owned. QI-14 only records its **cost**: `normalize_levels_config` merges `DEFAULT_LEVELS_SETTINGS`, so omitted `prev30m_vwap_enabled` is **True** on the CAI fixture (QI-14-04).
...
| QI-14-04 | both | Design limitation | Medium | Verified | Product-default `prev30m_vwap_enabled=True` is silently merged onto sparse/CAI configs |
...
No Critical. No High. Design-limitation rows stay ≤ Medium (plan §2 rule 5). None of these invert `AUDIT_FINAL` §5 or AH §2.
...
3. **Study workers 1/2/4 emit identical per-cell `bundle_hash`.** The only index delta is `cache_outcome` (`cold` on w1 cell 0 vs `levels_hit` when the shared `/tmp` store is warm). That is cache provenance, not a second experiment. **Not** a claim that cell expectancy is a validated edge.
4. **CAI warm ↔ cold canonical bundle hashes are equal** on small and on the current realistic path (`f8a5505a…` / `311d6204…`).
5. **AppTest page reruns on a loaded 780-bar session do not raise.** First render ≤ 0.315 s (Levels). Session pickle 0.818 MB ≪ 400 MB websocket cap.
...
| QI-2 | H4: `DEFAULT_LEVELS_SETTINGS.prev30m_vwap_enabled=True` is why CAI/sparse API configs pay `compute_prev30m_vwap_levels`. Rolling POC typical-price `_rolling_poc` is dead; tick gate is the live path. |
...
| QI-10 | Loaded-session AppTest times above. Empty-page times match QI-10’s 0.12–0.17 s band. |
...
cProfile `compute_levels` (one run, 0.440 s): `compute_prev30m_vwap_levels` cumtime **0.163 s**.
