...
Probe script lives at `/tmp/qi13_probes.py` (pasted in §10). Transcripts stay under `/tmp/qi13-evidence/`. Nothing from `/tmp` is committed.

---

## 1. Scope actually covered (files) and anything skipped (why)

**Covered** (QI-0 exclusive ownership: every markdown under `docs/**`, root `README.md`, `.cursor/rules/thesistester.mdc`)

| Shelf | Count | Lines (this tree) |
|---|---:|---:|
| `docs/**/*.md` | **78** | **46,277** |
| Living Primary (`docs/README.md` table) | 25 named files (3 also in Contracts) | — |
| Normative contracts | 32 named files | — |
| `docs/archive/` | 11 (index matches `archive/README.md`) | — |
| `docs/research/` | 3 (index matches `research/README.md`) | — |
| `docs/quality/` | 12 (QI-00…QI-06, QI-10…QI-12, this report, `README.md`, `findings.csv` is not `.md`) | — |
| Root `README.md` | 1 (Help-allowlisted `whole_file`) | 347 |
| `tests/fixtures/golden/README.md` | 1 (QI-13-owned spec; QI-11 reads) | — |
| `.cursor/rules/thesistester.mdc` | 1 | 23 |
...
| Import/call-ban sentences (must not import/call) | **9** explicit (Study + journal) | QI-12-07 already encoded 6 documented import bans |
| Line-number citations in `AGENT_GUIDE` | **7** real `file:line` cites (`README.md:7-10` ×2, `README.md:12-16`, `app.py:10-33`, `validation.py:13`, `pages/10_Validation.py:18`, `reporting.py:13-19`, `backtest.py:12-14`) | ARCHITECTURE forbids line anchors |
| Intra-doc markdown links | **169** local OK + **155** external; **0 missing** | — |
...
The two densest sections are the Study/Assistant rule ledgers. Those prose bans are the import-linter candidates (QI-12-07 measured 3/6 documented bans broken only via *chains*). Finding QI-13-06.
...
|---|---|---|
| Primary table (25) | living | Index exists |
| Contracts list | complete — amend carefully | Index exists |
...
Re-measured on `539dd2e` (QI-10 was `32ad34c`; no product files changed for this graph):

| | Count |
|---|---:|
| Main contract table names | **109** |
| `session_state` string literals (`app.py` + 15 pages + `classic_nav` / `timezone_display` / `app_state`) | **164** |
...
- Contract “Files allowed to touch” backtick paths: **383** resolve. The 10 “missing” hits are false friends (dotted module names, pytest node ids, gitignored `config/assistant.voice.override.toml`, the word `configure`). **No renamed/removed file** in those lists.
...
|---|---|---|
| `AUDIT_FINAL` §5.5 / AH2 “AGENT_GUIDE L38–39 advertising replay” | this slice | **Closed-verified (docs).** `AGENT_GUIDE` Fast-start Study block states pinned bytes + `run_batch` + “not study run”. `STUDY_RUNNER.md` same. Phrase “unchanged R18 path” is **gone**. Pin math / promote roots remain QI-7 (H2). Do not re-audit C2. |
...
5. **Contract “Files allowed to touch” lists do not point at deleted paths** (383 resolvable; 10 regex false friends).
...
|---|---|---|
| `preview.py` must not import `execute` | import-linter / AST test | Direct ban **holds**; chain via `expand`→`cli` is the residual |
...
| Library Streamlit-free except documented chrome | import-linter layers | `app_state.py` + lazy `classic_*` (QI-06-05 / QI-12-07) |
| “CI jobs are blocking” | required-status check (QI-12) | **False** today — QI-13-01 |
| `file:line` citations | delete or path-only (ARCHITECTURE rule) | 7 cites — QI-13-06 |
...
```python
# /tmp/qi13_probes.py — inventory, markdown-link checker, AGENT_GUIDE ledger,
# Help allowlist vs H2, ARCHITECTURE key table vs session_state literals,
# METRICS_GLOSSARY needles, ROADMAP QI/CI strings, README trigger list,
# Files-allowed existence, AH2 wording, USER_GUIDE DA0/H7/H8 flags.
# Full script: /tmp/qi13_probes.py. JSON: /tmp/qi13-evidence/summary.json
```
...
```bash
rg -n "not study run|unchanged R18 path|run_batch" docs/AGENT_GUIDE.md docs/STUDY_RUNNER.md
```
