...
- `thesistester/classic_record.py`: `record_classic_session_run` builds the
  ...
  lineage file. The export overlay also coerces timestamp units to the
  canonical loader's `us` resolution so 15s→1m derived parents do not fail
  `source_path_identity_mismatch` on dtype-only hash drift.
