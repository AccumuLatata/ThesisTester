...
**Status:** RS-D5 ✅  
**Depends on:** RS1–RS5 MVP, **RS-D7** (index `profit_factor` / `win_rate`), **RS6** (default-off `STUDY.*` + minimal confirm recipe), benefits from RS-D2 viewer + RS-D4 rollup  
...
| Studies viewer (RS-D2) | Human inspect | Read-only artifacts; no expand/run/promote |
