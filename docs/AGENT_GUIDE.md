...
- **A-period POC parity (AP):** `docs/APOC_QUANTOWER_INVESTIGATION_PLAN.md` (AP3 implemented). Default `apoc_profile_source` is `typical_mvp_v1`. Opt-in `tick_last_volume_v1` is the Quantower A-period source; do not ship a bar-range proxy as Quantower-compatible. Tick math is reused from `apoc_candidates` via `apoc_tick.py`. Do not reuse `PriorProfileTable` as APOC. `run_experiment` / `compute_levels` must forward `dataset.tick_paths` into the A-period table even when a prior-VA parquet is present. Program B Wave 7 is labeled legacy typical-price; do not rewrite historical ZIPs or put `apoc_profile_source` on committed Wave 7 YAMLs (identity lock). **Rolling POC is RP, not AP.**
- **Rolling POC parity (RP):** `docs/ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md` (RP0 locked, rev 2; RP1–RP2 scoped). Next is RP1 harness (`thesistester/levels/rolling_poc_candidates.py`); do **not** edit `_rolling_poc` **body**. Do not treat AP1’s 4/4 as rolling-POC evidence. Typical at 09:59 equals `_compute_a_period_poc(members)` / `APOC` at **10:00** (09:59 `APOC` is NaN). Tick/15s print window is `[now-W+1min, now+1min)`, not `min/max(members)`. §4.3 gate is **exactly 10** predeclared stamps. New kwargs are trailing with defaults (no `*` on `compute_all_levels`). Do not reuse `PriorProfileTable` / `APeriodTickProfileTable`. RP2 (`rolling_poc_profile_source`) merges only after RP1g selects Last×Volume **or** 1m `bar_range_uniform_volume_v1`; omitted key stays typical; no `LEVEL_ENGINE_VERSION` bump; no Program B YAML; no golden regen. Two-pointer incremental histogram only — do not call `compute_tick_last_volume_profile` per bar. Identity keys strip in `api.py` / `research_identity.py` / `classic_export.py`.
