...
- **Named-suite convention (B-16 / QI-11-06).** Accept-path tests assert the
  ...
  re-acquire / `_fsync_file` swallows are the QI-11 §2.5 twelve. A
  loosened validator or a no-op helper must fail those asserts. C-1 /
  C-3 (validator refactors) may land only while
  `tests/test_qi11_accept_asserts.py` stays green. Fixture builders
  (`_bar` / `_ohlcv` / `_signal`) stay in the suite that owns the
  contract; do not consolidate them in the same PR as a validator or
  engine change.
