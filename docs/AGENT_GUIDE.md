...
- **A-period POC parity (AP):** `docs/APOC_QUANTOWER_INVESTIGATION_PLAN.md` (AP2 implemented). Default `apoc_profile_source` is `typical_mvp_v1`. Opt-in `tick_last_volume_v1` is the Quantower A-period source; do not ship a bar-range proxy as Quantower-compatible. Tick math is reused from `apoc_candidates` via `apoc_tick.py`. Do not reuse `PriorProfileTable` as APOC. `run_experiment` / `compute_levels` must forward `dataset.tick_paths` into the A-period table even when a prior-VA parquet is present. Do not bundle AP3 Program B provenance with this source PR.
...
- **Journal → Study (JS):** `docs/JOURNAL_TO_STUDY_IMPLEMENTATION_PLAN.md` (JS0 locked; plan only). Zone attribution (`detect_confluence_zones` on the previous completed 1m bar) + trigger inference (`classify_zone_triggers` wrapper, JS2) + frequency-selected StudySpec with fail-closed `holdout` + TJ8 multi-bundle rule-vs-desk. Gate A can stop the series after JS2. Do not add a 15s trigger lane, edit `_check_*` bodies, regenerate goldens, select factors by outcome, or auto-run the proposed study. No new USER_GUIDE H2. Do not implement JS inside a TJ/DA PR.
