...
- **Rolling POC parity (RP):** `docs/ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md` (RP0 locked; RP1–RP2 scoped). Next is RP1 harness (`rolling_poc_candidates.py`); do not edit `_rolling_poc` in RP1. Do not treat AP1’s 4/4 as rolling-POC evidence. Do not reuse `PriorProfileTable` / `APeriodTickProfileTable`. RP2 (`rolling_poc_profile_source`) merges only after RP1g selects Last×Volume; omitted key stays typical; no `LEVEL_ENGINE_VERSION` bump; no Program B YAML; no golden regen. Two-pointer tick path only — no nested tick scan.
