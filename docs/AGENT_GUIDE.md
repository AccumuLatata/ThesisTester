...
- Route assistant compute only through `AssistantOrchestrator` façade methods
  ...
  otherwise mark it `unsupported` with a limitation. Every routed capability
  ID must have a dispatch or payload test in `tests/test_assistant_handlers.py`
  (QI-09-04 / B-8), or an explicit untestable tag; `get_handler` is unit-tested
  there. Structured errors must
