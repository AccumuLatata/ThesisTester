...
C-5 is reporting (`build_markdown_report` section table). Next: C-6.
Stage-first example:
...
3. `build_setup(...) -> dict` applies the Setup Builder normalization and
   ...
   `MARKDOWN_REPORT_SECTIONS` (own table; A-4 H13 banner stays the first
   line under `## Validation Diagnostics`). Next: C-6.
...
- **Phase 9 (Report/Export):** `pages/11_Report_Export.py`, artifact builders in `thesistester/reporting.py`. C-5 (QI-06-02): `build_markdown_report` walks `MARKDOWN_REPORT_SECTIONS`; emitted markdown is fixture-locked.
