...
- **Trade journal (TJ):** `docs/TRADE_JOURNAL_IMPLEMENTATION_PLAN.md` (TJ0 locked). Quantower Trades CSV + AMP Daily Statement PDF only. Do not commit desk PII exports. Do not edit `simulate_trades`. QT `Fee`/`Gross P/L` are unused; AMP is the fee SoT. Same-day AMP↔QT overlap golden is still desk-blocked.
