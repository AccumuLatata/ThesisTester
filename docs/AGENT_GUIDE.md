...
- **Rolling POC parity (RP):** `docs/ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md` (RP2 default tick Last×Volume; desk amendment). Production `POC_rolling_*` is two-pointer Last×Volume on `[now-W+1min, now+1min)`. Missing ticks → all-NaN, never typical. Do **not** edit `_rolling_poc` **body** (dead/non-default). Do not treat AP1’s 4/4 as a QT rolling scorecard. Do not reuse `PriorProfileTable` / `APeriodTickProfileTable`. Do not open RP2-cancel. APOC default stays typical. No `LEVEL_ENGINE_VERSION` bump; identity keys stamp tick. No Program B YAML key (NaN rolling there without ticks). No golden regen.
