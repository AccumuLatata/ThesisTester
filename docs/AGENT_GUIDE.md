...
- **Trade journal (TJ):** `docs/TRADE_JOURNAL_IMPLEMENTATION_PLAN.md` (TJ0 locked). TradesViz executions CSV (Layer 1, UTC) + AMP Daily Statement PDF (Layer 2, fees) only; Quantower loader parked (Vienna-local clock). Do not commit desk PII exports. Do not edit `simulate_trades`. TradesViz `commission`/`fees` are unused; AMP is the fee SoT. Tags are intent — verify against the levels frame, never treat as a trigger. TJ7 counterfactuals are pure functions over bars/ticks (never `simulate_trades`); the only RNG is the seeded direction-shuffle null; discipline rules are declared with a date, never searched. Do not rebuild generic journal stats TradesViz already provides.
