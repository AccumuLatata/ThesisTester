...
- **Rolling POC parity (RP):** `docs/ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md` (RP0 locked). `POC_rolling_30min` remains 1m typical. Do not treat AP1’s 4/4 A-period tick scorecard as rolling-POC evidence (window equals A-period only at 09:59). Do not reuse `PriorProfileTable` / `APeriodTickProfileTable` as a rolling window. Do not ship a bar-range proxy as Quantower-compatible. RP2 tick opt-in is no-go until a written sliding-window oracle. RP1 comparator must not change `_rolling_poc` output.
