...
**Related follow-on (do not implement here):** Study Ingest Alignment — `docs/STUDY_INGEST_ALIGNMENT_IMPLEMENTATION_PLAN.md` (SIA). Authoring/defaults only; does not reopen SB execute/preview/launch contracts. Study Viewer — `docs/STUDY_VIEWER_IMPLEMENTATION_PLAN.md` (SV). Inspect catalog/quality/charts/peek only; does not reopen SB emit/Apply/Preview. Study Admit Follow-up — `docs/STUDY_ADMIT_FOLLOWUP_IMPLEMENTATION_PLAN.md` (SAF). Child Admit drafts via promote; does not reopen SB emit/Apply.
