...
| 6 | `deep_trade` | RI-9 | `exit reason`/`exit reasons`/`exit-reason(s)`, `why/how did trades exit`, `how many trades exited` / `trades exited`, `worst trade(s)` / `best trade(s)` / `extreme trades`, `win/loss streak(s)` / `winning/losing streak`, `consecutive wins/losses`. Answers only from capped ephemeral §6 projections — never raw trade frames. Only the collocate ``how many trades exited`` steals `trade_count` (bare ``exit``/``exited`` must not black-hole incidental phrasing or and-mixed asks). Exit/extreme cues require table-derived projections; streak cues may use `trade_summary` streak scalars. Shared helper `build_deterministic_discuss_reply` (RI-10) must forward `user_message` for topic-scoped §6 gates. |
