...
strips accidental `evidence_packet.` / `packet.` claim-path prefixes and
resolves JSON array indices; fractional rates accept `%` or word-form
percent narration. Optional RO `TIME.analyze` enrichment runs only when
