...
setup dict through `build_setup_config` (C-4 / QI-03-10) via
`build_setup_kwargs_from_mapping`. It still calls engine `generate_signals`
...
**DA2** adds study-owned direction-split keys on `STUDY_INDEX_KEYS` only
(not `R18_INDEX_METRIC_KEYS`): long/short n and E, `long_share`,
...
**SV1–SV5** (Study Viewer catalog + quality panes + overview charts + cell peek
+ trader briefing, `docs/STUDY_VIEWER_IMPLEMENTATION_PLAN.md`) add Inspect
local-study listing + click-to-load, additive CLI `study list`, read-only
failed-cell / group / rollup-if-present / launch-log panes, locked Plotly from
already-loaded ranked / group frames, one-cell index / ledger-error / optional
`trade_summary.json` peek, and SV5 briefing + one-cell SL/TP grid / NY RTH
ToD from zip members (`thesistester/study/briefing.py`). They keep
`report_study(..., write_artifacts=False)`, must not call `rollup_study()`
(writes), must not hydrate classic research `st.session_state` keys, must not
unzip every cell, and must not deep-link Research Bundles. Discover does not
call `report_study`. Time-of-day stays a post-hoc projection, not a new
StudySpec factor.
...
Refuse is `st.error` via `studies_admit_followup_error`; success flash is
...
after) and rejects a **named** member whose `ZipInfo.file_size` exceeds
`MAX_BUNDLE_MEMBER_BYTES` **before** `ZipFile.read`. `peek_research_identity`
...
serial baseline for future parity-gated work (E-10).
...
repairs loaded fields with C-1 `validate_setup_config` enum/range sets
(`VALID_TRIGGERS`, `VALID_DIRECTIONS`, `VALID_CONFLUENCE_MODES`), then writes
`_setup_builder_*` keys from `build_setup_config` outputs (name/description
strip, trigger_params, OTF, entry_window). A failed canonicalize keeps the
repaired payload and warns — it does not mix in product defaults. Render
reads the keys and does not sync. `_safe_*` stays on the render path.
...
MC drawdown parse and Phase 8 formatters live in
`validation_page_helpers.py` (`parse_thresholds`, `fmt_value`); H13
`p_val > 0.05` else `st.info` + caption literals stay on the display
helper (assembler is the unit-tested twin).
...
The flag is cleared on Data-page successful load (`_set_active_dataset_state`).
...
(tick-family preflight and other typed engine refuse) store `error_message` only and
render `st.error` without a traceback expander. Unexpected exception types still
attach `traceback` and keep the Calculation diagnostics expander (`st.code`).
