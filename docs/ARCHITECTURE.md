...
evict helpers live in `execution_artifact_ops.py` and run only after
`_contain_path` / `_assert_path_under_execution_artifacts`. Cache-policy
default stays `off`. Warm `read_write` reuse must produce the same
content-addressed hashes as a cold compute.
