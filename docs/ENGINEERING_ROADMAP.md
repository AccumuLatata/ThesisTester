...
| Research Study Runner | `docs/STUDY_RUNNER_IMPLEMENTATION_PLAN.md` (RS); operator `docs/STUDY_RUNNER.md` | ✅ **RS1–RS5 MVP complete**; post-MVP sequenced **RS6 → RS-D7 → RS-D2 → RS-D4 → RS-D5** (plan-locked) |
...
| **Post-MVP sequence** | **RS6 → RS-D7 → RS-D2 → RS-D4 → RS-D5** (do not reorder without plan amend) |
| RS6 | Default-off assistant/MCP study tools + Grok CLI/MCP docs ☐ next |
| RS-D7 | Additive `results_index` `profit_factor` / `win_rate` ☐ |
| RS-D2 | Streamlit Studies **viewer** (artifacts-only; no in-app run) ☐ |
| RS-D4 | Study-aware WFA/PBO rollup (compose existing batteries) ☐ |
...
continue-capable ledger + soft resume); RS6 tools **default-off** with confirm
parity; RS-D2 **read-only** viewer; RS-D7 additive/default-compatible index
columns; RS-D4 compose-only (no new inference); combinatorial fishing mitigated
by confirm gates, stage-first examples (40 vs 800), and multiple-testing honesty.
