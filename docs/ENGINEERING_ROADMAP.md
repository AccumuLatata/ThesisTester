...
| Quality investigation (code + application) | `docs/QUALITY_INVESTIGATION_PLAN.md` (QI); slice reports `docs/quality/QI-*.md` + `findings.csv` | **Planned.** QI-0 baseline measured on `e82c2a9` (§4). **Precondition:** `main` CI red since 2026-09-05 (Streamlit 1.63 `AppTest` + pandas 2.3/3.0 matrix split; plan §4.3/§4.6 hotfix) must be cleared first. QI-1…QI-14 research-only slices `Not started`; QI-15 synthesis drafts the QR remediation plan (separate approval). Carries over open `AUDIT_FINAL` H-items by status only; `AUDIT_FINAL` §5 / AH §2 stay locked; no code, golden, or contract changes in QI |
