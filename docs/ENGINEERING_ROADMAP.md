...
| Research intelligence (specialist Discuss slices) | `docs/RESEARCH_INTELLIGENCE_IMPLEMENTATION.md` (RI) | 🚧 **RI-6 landed** — grid + time + validation/WFA + single-metric + meaning overlay + mixed-ask composition + tier-2 robustness + assumptions/costs (§4.6); residual DI veto (§4.1.1); next `RI-9→RI-10`; identical RQ auditor; no engine/golden touch; series not complete until RI-10 |
