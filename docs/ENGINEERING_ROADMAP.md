...
| Level-combination research concept | `docs/LEVEL_COMBINATION_RESEARCH_CONCEPT.md` · inventory `docs/LEVEL_VS_MA_VWAP_PIVOT_INVENTORY.md` | **Program B** (docs-only, not executed). 50 location cores × MA / rolling VWAP / pivot. `dVWAP` is an optional core, not a required partner. Does not amend the Notion desk lock page |
