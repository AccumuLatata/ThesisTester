...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA1 landed** (DA0 locked). In-memory `direction_collision_diagnostic` on `SimulationResult` / `run_backtest` / `run_experiment`; not hashed. Series code is **DA** (DI is Discuss Intelligence). Next: DA2 direction split + Run 1 re-read → DA3 opt-in same-bar policy → DA4 `fade`/`continuation` → DA5 drift null → DA6 Run 2. Additive, default-legacy; no golden regen |
