...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA2 landed** (DA0 locked, DA1 landed). Study-index long/short n and E + `directional_integrity`; `--rebuild-direction` on `study report`; Observatory facet/banner/`L n / S n` tooltip. Series code is **DA** (DI is Discuss Intelligence). Next: DA3 opt-in same-bar policy → DA4 `fade`/`continuation` → DA5 drift null → DA6 Run 2. Additive, default-legacy; no golden regen |
