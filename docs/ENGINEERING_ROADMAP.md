...
| Duplex intelligence (realtime content parity) | `docs/DUPLEX_INTELLIGENCE_IMPLEMENTATION.md` (DX) | 🟡 **DX-0 ✅ / DX-1 in flight** — DI-shaped `get_run_overview` envelopes + `has_overview_negative_cue` + intent sample-size → `trade_summary.trade_count` + speakable `summary`-first; DX-2 instructions + DX-3 eval freeze pending; voice default remains off |
