...
| Audit honesty remediations | `docs/AUDIT_HONESTY_IMPLEMENTATION_PLAN.md` (AH) | **AH0–AH3.** AH4–AH6 specified. Flatten leak fixed; Study dataset paths pinned; OTF-matrix train prices prefix-sliced. Next: restore leftovers → 3c `sl_first` clip → `BASE_COLUMNS` validator. No composer collapse; no golden regen except AH5 hard-stop |
