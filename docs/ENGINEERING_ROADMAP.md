...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA5 landed** (DA0 locked, DA1–DA4 landed). Opt-in study `report.random_baseline` drift null on the index + Observatory `drift_class`. Series code is **DA** (DI is Discuss Intelligence). Next: DA6 Run 2. Additive, default-legacy; no existing-golden regen |
