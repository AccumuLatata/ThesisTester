...
| Research Assistant page layout / prominence | `docs/RESEARCH_ASSISTANT_UX_REFOCUS_PLAN.md` (RUX) | 🟡 **In progress** — RUX-0…RUX-4 landed ([#304](https://github.com/AccumuLatata/ThesisTester/pull/304): Help remediation / discuss-run corpus re-anchor); RUX-5 closeout planned. Presentation-only: must not change engine, `thesistester.api`, orchestrator/repository/registry, or RQ channel logic |
