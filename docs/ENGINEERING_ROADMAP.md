...
| Research intelligence (specialist Discuss slices) | `docs/RESEARCH_INTELLIGENCE_IMPLEMENTATION.md` (RI) | 📋 **RI-0 contract freeze** — continuation of DI: fail-open specialist/single-metric/meaning/mixed-ask slices with identical RQ auditor; residual DI veto migration (§4.1.1); ship `RI-0→RI-1→RI-3→RI-2→RI-4→…→RI-10`; no engine/golden touch; series not complete until RI-10 |
