...
| Research intelligence (specialist Discuss slices) | `docs/RESEARCH_INTELLIGENCE_IMPLEMENTATION.md` (RI) | 🚧 **RI-7 landed** — grid + time + validation/WFA + single-metric + digit-free meaning overlay (§5); residual DI veto (§4.1.1); next `RI-8→…→RI-10`; identical RQ auditor; no engine/golden touch; series not complete until RI-10 |
