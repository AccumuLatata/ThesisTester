...
| Research intelligence (specialist Discuss slices) | `docs/RESEARCH_INTELLIGENCE_IMPLEMENTATION.md` (RI) | 🚧 **RI-8 landed** — grid + time + validation/WFA + single-metric + meaning overlay + mixed-ask composition (§4.7, ≤3 intents); residual DI veto (§4.1.1); next `RI-5→…→RI-10`; identical RQ auditor; no engine/golden touch; series not complete until RI-10 |
