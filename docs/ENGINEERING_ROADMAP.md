...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA4 landed** (DA0 locked, DA1–DA3 landed). Opt-in `fade` / `continuation` approach-side triggers; new `fade_enabled_*` golden. Series code is **DA** (DI is Discuss Intelligence). Next: DA5 drift null → DA6 Run 2. Additive, default-legacy; no existing-golden regen |
