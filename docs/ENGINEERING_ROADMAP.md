...
| Rolling POC Quantower parity | `docs/ROLLING_POC_QUANTOWER_INVESTIGATION_PLAN.md` (RP) | **RP0 locked (rev 2); RP1–RP2 scoped.** Next is RP1 comparator (no production math). Print window is `[now-W+1min, now+1min)`, not `min/max(members)`. Do not edit `_rolling_poc` body. RP2 opt-in is merge-gated on RP1g (tick **or** 1m bar-range). Default stays typical. No golden regen. |
...
## Rolling POC Quantower parity (RP0–RP2) — RP0 locked (rev 2), RP1–RP2 scoped
