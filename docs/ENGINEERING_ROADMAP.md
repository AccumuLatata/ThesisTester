...
- **Lint/format** — deliberately minimal ruff rule set (`E4`, `E7`, `E9`, `F`, `W`) at line
  length 100, with two documented per-file ignores: `E402` for `pages/1_Data.py`
  only (QI-10-08 / B-14; that page bootstraps `sys.path` before importing
  `thesistester`) and `E741` for test fixtures using `l`/`O` bar shorthand.
