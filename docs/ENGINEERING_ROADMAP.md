...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DI) | **DI0 proposed.** Verified: `touch` + `direction: both` + `single_position` accepts only longs (same-bar short skipped by `signal_id` tie-break) — Program A L1 and Program B Run 1 are long-only samples. DI1 collision diagnostic → DI2 direction split + Run 1 re-read → DI3 opt-in same-bar policy → DI4 `fade`/`continuation` triggers → DI5 drift null → DI6 Run 2 packet. Additive, default-legacy; no golden regen |
