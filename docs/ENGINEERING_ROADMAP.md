...
| Quality investigation (code + application) | `docs/QUALITY_INVESTIGATION_PLAN.md` (QI); slice reports `docs/quality/QI-*.md` + `findings.csv` | **Planned.** **16 slices (QI-0…QI-15).** QI-0 baseline measured on `e82c2a9` (§4): 153 library modules; 196 test `.py` files; 3,715 `def test_` / 3,966 collected; 18,172 LOC in `pages/` only; eight MI-0.00 pages plus `analytics/confluence_attribution.py`; 0 real TODO/FIXME/XXX/HACK; CLI `python -m thesistester study\|journal --help`; four golden families; six ad-hoc CI branches; highest CC `reporting.build_markdown_report` 152; 1,176 `session_state` matching lines; exclusive file ownership; research-only except QI-0 harness + QI-15 `QUALITY_REMEDIATION_PLAN.md`. **Precondition (separate PR, not QI):** `main` CI red since 2026-09-05 (Streamlit 1.63 `AppTest` + pandas 2.3/3.0 matrix split; plan §4.3/§4.6) must be cleared first. QI-1…QI-14 `Not started`; QI-15 synthesis drafts the QR plan (separate approval). Carries over open `AUDIT_FINAL` H-items by status only; `AUDIT_FINAL` §5 / AH §2 stay locked; no code, golden, or contract changes in QI |
