...
| Audit honesty remediations | `docs/AUDIT_HONESTY_IMPLEMENTATION_PLAN.md` (AH) | **AH0 + AH1.** AH2–AH6 specified. Flatten leak fixed (per-candidate `entry_local_ts`). Next: Study path pin → OTF-matrix train slice → restore leftovers → 3c `sl_first` clip → `BASE_COLUMNS` validator. No composer collapse; no golden regen except AH5 hard-stop |
