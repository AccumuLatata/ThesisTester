...
| Voice review | `docs/REALTIME_VOICE_AGENT_IMPLEMENTATION.md` (VA) | **In progress** — VA-0 ✅ (contracts/flag/docs freeze); VA-1 done via RQ; Help substrate = RQ channel + HC corpus (both ✅); remaining VA-2…VA-6 (session/STT/TTS → tools → PTT → realtime → evals; do not reopen HC) |
