...
| Audit honesty remediations | `docs/AUDIT_HONESTY_IMPLEMENTATION_PLAN.md` (AH) | **AH0 plan lock.** AH1–AH6 specified, not implemented. Flatten leak → Study path pin → OTF-matrix train slice → restore leftovers → 3c `sl_first` clip → `BASE_COLUMNS` validator. No composer collapse; no golden regen except AH5 hard-stop |
...
| Milestone | Intent |
|---|---|
| AH0 | Plan lock + docs index (this row) |
...
AH2 path-pin / AGENT_GUIDE replay honesty.
