...
| Duplex intelligence (realtime content parity) | `docs/DUPLEX_INTELLIGENCE_IMPLEMENTATION.md` (DX) | 🟡 **DX-0 plan** — follow-on to DI for VA-5 full-duplex **content** parity via tool envelopes + instructions (reuse DI builders; veto ≠ unmatched via `has_overview_negative_cue`; veto strips legacy explainer narrative; session-transcript selector + acknowledged race; intent sample-size → `trade_summary.trade_count`; no provider swap; no live-PCM pre-gate; voice default remains off) |
