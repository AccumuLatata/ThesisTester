...
| Level Catalog Contract | `docs/LEVEL_CATALOG_CONTRACT_IMPLEMENTATION_PLAN.md` (LC) | **LC3 landed.** LC4 not started. Catalog completeness/correctness for already-emitted levels. No new price series; no `LEVEL_ENGINE_VERSION`; no golden regen |
...
## Level Catalog Contract (LC0–LC4) — LC3 landed
...
`4h`; widget catalogs stay widget-only). LC4 will fail-close API
global-cluster missing columns. Implement **one PR per defect family**. Do not
