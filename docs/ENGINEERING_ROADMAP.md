...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA3 landed** (DA0 locked, DA1–DA2 landed). Opt-in `same_bar_opposite_direction` (`legacy` / `skip_both` / `raise`; default legacy). Series code is **DA** (DI is Discuss Intelligence). Next: DA4 `fade`/`continuation` → DA5 drift null → DA6 Run 2. Additive, default-legacy; no golden regen |
