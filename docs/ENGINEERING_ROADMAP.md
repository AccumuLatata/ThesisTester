...
| Study Viewer (Inspect UX) | `docs/STUDY_VIEWER_IMPLEMENTATION_PLAN.md` (SV); operator `docs/STUDY_RUNNER.md` §SV | ✅ **SV0 + SV1 + SV2**. Catalog + `study list` + quality panes. SV3–SV4 not started. No in-process execute; no classic-session hydrate |
...
## Study Viewer (SV0–SV4) ✅ SV2 shipped

Inspect today is catalog + path-paste + tables + quality panes
(`report_study(..., write_artifacts=False)`). SV1 lists local study dirs and
reuses the existing Load path. SV2 projects failed-cell errors, group
summaries, optional rollup files, and a launch-log tail. SV3–SV4 add
