...
| Directional integrity & edge attribution | `docs/DIRECTIONAL_INTEGRITY_IMPLEMENTATION_PLAN.md` (DA) | **DA0 locked.** Series code is **DA** (DI is Discuss Intelligence). Verified: `touch` + `direction: both` + `single_position` accepts only longs (same-bar short skipped by `signal_id` tie-break) — Program A L1 and Program B Run 1 are long-only samples. DA1 collision diagnostic → DA2 direction split + Run 1 re-read → DA3 opt-in same-bar policy → DA4 `fade`/`continuation` triggers → DA5 drift null → DA6 Run 2 packet. Additive, default-legacy; no golden regen |
