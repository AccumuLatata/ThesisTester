...
**Related follow-on (do not implement here):** Study Admit Follow-up — `docs/STUDY_ADMIT_FOLLOWUP_IMPLEMENTATION_PLAN.md` (SAF). Drafts a child Admit StudySpec from briefing ToD; does not reopen SV briefing / peek / catalog contracts. Study Observatory — `docs/STUDY_OBSERVATORY_IMPLEMENTATION_PLAN.md` (SO). Corpus fact table + new page 16; reuses SV1 discover; must not grow Inspect or call `report_study` per study.
