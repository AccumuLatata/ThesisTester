...
def build_evidence_packet(
    ...
    for label, cost_key, exposure_key in (
        ("Backtest", "backtest_execution_costs", "exposure_policy"),
        ("Grid", "grid_execution_costs", "grid_exposure_policy"),
    ):
        costs = state.get(cost_key)
        if (
            isinstance(costs, Mapping)
            and costs.get("commission_per_side", 0) == 0
            and costs.get("slippage_ticks", 0) == 0
        ):
            warnings.append(f"{label} uses zero commission and zero slippage assumptions.")
        exposure = state.get(exposure_key)
        if isinstance(exposure, Mapping) and exposure.get("exposure_policy") == "allow_all":
            warnings.append(
                f"{label} allow-all exposure can count overlapping signals independently."
            )
