...
def _project_zones_to_trigger_df(
    ...
    # Build a lookup: base bar index -> trigger-row position (for each base bar
    # in any trigger bar). Column arrays avoid iterrows (QI-14-05 / C-15).
    starts = tdf["base_start_bar_index"].to_numpy()
    ends = tdf["base_end_bar_index"].to_numpy()
    trigger_by_any_base: dict[int, int] = {}
    for i in range(len(tdf)):
        start = int(starts[i])
        end = int(ends[i])
        for b_idx in range(start, end + 1):
            if b_idx not in trigger_by_any_base:
                trigger_by_any_base[b_idx] = i

    zones_reset = zones.reset_index(drop=True)
    base_idxs = zones_reset["bar_index"].to_numpy()
    projected_rows: list[dict] = []
    for i in range(len(zones_reset)):
        tpos = trigger_by_any_base.get(int(base_idxs[i]))
        if tpos is None:
            continue
        trow = tdf.iloc[tpos]
        projected = dict(zones_reset.iloc[i])
...
# ---------------------------------------------------------------------------
# generate_signals phases (QI-03-01 / C-14)
# ---------------------------------------------------------------------------
...
def _prepare_generate_trigger_frame(
    ...
    return df_reset, trigger_df, _index_trigger_rows_by_base_end(trigger_df)


def _admit_zones_for_signals(
    zones: pd.DataFrame,
    ...
    *,
    ...
    """Admit zones that have a trigger bar and satisfy the naked filter."""
    ...
    for i in range(len(zones)):
        ...
        if bar_idx >= n_df:
            continue
        if not is_3c and bar_idx not in trigger_rows_by_base_end:
            continue
...
def _zone_and_naked_for_3c_setup(
    setup: dict,
    zone_by_id: dict,
    naked_flags: pd.DataFrame | None,
) -> tuple[pd.Series, int]:
    """Resolve zone Series + arrival naked count for one 3c setup (C-16)."""
...
def _map_3c_setup_to_signal(
    *,
    ...
    entry_idx_base = (
        _safe_signal_index(entry_bar_index, len(df_reset)) if entry_bar_index is not None else None
    )
    ...
    if trigger_df is None:
        ...
        trigger_arrival_bar_index = (
            _safe_signal_index(trigger_arrival_raw, len(trigger_df))
            if trigger_arrival_raw is not None
            else None
        )
        trigger_reversal_bar_index = trigger_bar_index
...
def _generate_3c_signals(
    *,
    ...
    """Map 3c detector rows onto signal dicts (base and HTF paths)."""
    signals: list[dict] = []
    signal_id = 0
    ...
    if effective_trigger_timeframe == "base":
        ...
        for setup in setup_rows:
            mapped = _map_3c_setup_to_signal(
                ...
                naked_req=naked_req,
                df_reset=df_reset,
                effective_trigger_timeframe=effective_trigger_timeframe,
                trigger_df=None,
            )
            if mapped is None:
                continue
...
_SimpleTriggerChecker = Callable[
    [pd.DataFrame, pd.Series, int, int, str, str, int, int, str],
    dict | None,
]
_ApproachSideChecker = Callable[..., dict | None]

_SIMPLE_TRIGGER_CHECKERS: dict[str, _SimpleTriggerChecker] = {
    "touch": _check_touch,
    "reject": _check_reject,
    "break": _check_break,
    "reclaim": _check_reclaim,
}

_APPROACH_SIDE_CHECKERS: dict[str, _ApproachSideChecker] = {
    "fade": _check_fade,
    "continuation": _check_continuation,
}


def _dispatch_approach_side_triggers(
    trigger: str,
    filtered_zones: list[tuple[pd.Series, int]],
    trigger_df: pd.DataFrame,
    trigger_rows_by_base_end: dict[int, _TriggerRowRef],
    *,
    ...
    """Dispatch fade / continuation through the approach-side table."""
    signals: list[dict] = []
    signal_id = 0
    require_close = bool(params.get("require_close_confirmation", False))
    checker = _APPROACH_SIDE_CHECKERS[trigger]
...
def _dispatch_simple_triggers(
    ...
    *,
    ...
    """Dispatch touch / reject / break / reclaim through the simple table."""
    signals: list[dict] = []
    signal_id = 0
    directions = ["long", "short"] if direction == "both" else [direction]
    checker = _SIMPLE_TRIGGER_CHECKERS.get(trigger)
    for zone, ncount in filtered_zones:
        bar_idx = int(zone["bar_index"])
        trigger_row = trigger_rows_by_base_end.get(bar_idx)
        if trigger_row is None:
...
def generate_signals(
    ...
    df_reset, trigger_df, trigger_rows_by_base_end = _prepare_generate_trigger_frame(
        df, effective_trigger_timeframe
    )
    filtered_zones = _admit_zones_for_signals(
        zones,
        df_reset,
        trigger,
    ...
    if trigger == "3c":
        ...
        signals = _dispatch_approach_side_triggers(
            trigger,
            filtered_zones,
            trigger_df,
            trigger_rows_by_base_end,
        ...
        signals = _dispatch_simple_triggers(
            trigger,
            filtered_zones,
            trigger_df,
            trigger_rows_by_base_end,
...
def _classify_zone_triggers_detail(
    ...
    *,
    ...
    if prepared.empty:
        return (), {}
    trigger_rows_by_base_end = _index_trigger_rows_by_base_end(prepared)
