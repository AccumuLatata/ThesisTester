...
def _sample_session_state() -> dict:
    ...
    return {
        ...
        "sensitivity_summary": {
            ...
            "baseline": {"expectancy_r": 0.2},
            "parameters": [{"parameter": "stop_loss_ticks"}],
        },
...
def test_build_research_artifact_counts_match_signals_and_trades():
    ...
    assert artifact["results"]["sensitivity_summary"]["fragile_parameter_count"] == 1
...
def test_build_markdown_report_returns_string_and_required_sections():
    ...
    vd_lines = [ln for ln in vd_section.splitlines() if ln.strip()]
    assert vd_lines[0] == "## Validation Diagnostics"
    assert vd_lines[1] == ("⚠️ Diagnostic only — not a significance test and not proof of edge."), (
        f"banner must be the first line under the heading, got {vd_lines[1:]!r}"
    )
