...
def test_summarize_trades_by_direction_only_long():
    t = pd.DataFrame({"trade_id": [1, 2], "direction": ["long", "long"], "r_multiple": [1.0, -1.0]})
    out = summarize_trades_by_direction(t)
    assert out["long"]["trade_count"] == 2
...
def test_grouped_summary_empty_is_safe():
    grouped = summarize_by_group(
        pd.DataFrame(), ["trigger_variant", "level_source_mode", "direction"]
    )
...
def test_equity_curve_empty():
    ...
    assert curve.empty
    assert set(["trade_id", "exit_timestamp", "r_multiple", "cum_r", "drawdown_r"]).issubset(
        curve.columns
    )
