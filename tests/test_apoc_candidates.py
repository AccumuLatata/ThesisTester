...
from thesistester.levels.apoc import _compute_a_period_poc
...
def test_typical_candidate_matches_production_apoc_helper_without_routing():
    ...
    assert candidate.allocated_volume == pytest.approx(300.0)
...
def test_uniform_range_allocation_is_inclusive_and_conserves_volume():
    ...
    assert result.allocated_volume == pytest.approx(result.source_volume)
...
def test_sparse_bars_are_eligible_and_zero_volume_bars_do_not_allocate():
    bars = _bars(
        [
...
def test_empty_usable_bars_return_nan_without_a_typical_fallback():
    result = compute_bar_candidate_profile(
...
def test_tick_last_volume_conserves_volume_and_uses_lowest_bin_tie():
    ...
    assert result.allocated_volume == pytest.approx(8.0)
