...
def test_apoc_enabled_returns_exactly_apoc_and_papoc():
    df = _single_session_fixture()
    result = compute_apoc_levels(df, instrument="ES", enabled=True, apoc_profile_source="typical_mvp_v1")
...
def test_session_column_absent_derives_from_instrument_config():
    ...
    result = compute_apoc_levels(df_no_session, instrument="ES", enabled=True, apoc_profile_source="typical_mvp_v1")
...
def test_future_shock_appending_current_session_bars_does_not_change_apoc():
    ...
    result_base = compute_apoc_levels(df_base, instrument="ES", enabled=True, apoc_profile_source="typical_mvp_v1")
    ...
    result_extended = compute_apoc_levels(df_extended, instrument="ES", enabled=True, apoc_profile_source="typical_mvp_v1")
...
def test_future_shock_appending_next_session_bars_does_not_change_papoc():
    ...
    result_two = compute_apoc_levels(df_two, instrument="ES", enabled=True, apoc_profile_source="typical_mvp_v1")
...
def test_enabled_requires_supported_instrument():
    ...
    with pytest.raises(ValueError, match="Unsupported instrument"):
        compute_apoc_levels(df, instrument="UNSUPPORTED", enabled=True, apoc_profile_source="typical_mvp_v1")
...
def test_nq_instrument_supported():
    ...
    df = pd.DataFrame(rows)
    result = compute_apoc_levels(df, instrument="NQ", enabled=True, apoc_profile_source="typical_mvp_v1")
    assert set(result.columns) == {COL_APOC, COL_PAPOC}
